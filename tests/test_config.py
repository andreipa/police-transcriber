...
def test_is_model_downloaded_all_files_exist(temp_config_dir):
    ...
    assert is_model_downloaded("large-v2", base_path=str(model_dir.parent)) is True
...
def test_is_model_downloaded_missing_files(temp_config_dir):
    ...
    assert is_model_downloaded("large-v2", base_path=str(model_dir.parent)) is False
...
def test_save_config_success(temp_config_dir, mock_logger):
    ...
    with patch("config.CONFIG_FILE", str(config_file)):
        with patch("config.update_logging") as mock_update:
            ...
            mock_app.debug.assert_any_call(f"Saving config to: {os.path.abspath(str(config_file))}")
            mock_app.debug.assert_any_call(f"Successfully saved config to: {os.path.abspath(str(config_file))}")
